...
public class FilterTransactionCredit {
    ...
    public Mono<ProductDto> isSave(Transaction transaction){
        ...
        return findToProduct.filter(e-> e.getAccountType().equalsIgnoreCase(ConstantsCredit.CREDIT.name()))
    ...
    public Mono<ProductDto> isTypeTransfer(Transaction transaction,ProductDto productDto){
        ...
        if (Arrays.stream(ConstantsCreditTransac.values()).anyMatch(m->m.toString().equalsIgnoreCase(transaction.getTransactionType()))){
