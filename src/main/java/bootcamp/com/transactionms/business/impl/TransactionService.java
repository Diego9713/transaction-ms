...
public class TransactionService implements ITransactionService {
    ...
    public Flux<TransactionDto> findAllTransaction() {
        log.info("FindAll >>>");
        return transactionRepository.findAll().map(AppUtils::entityToTransactionDto);
    }
    ...
    public Mono<TransactionDto> findByIdTransaction(String id) {
        log.info("FindById >>>");
        return transactionRepository.findById(id).map(AppUtils::entityToTransactionDto).filter(transactionDto -> transactionDto.getStatus().equalsIgnoreCase(ConstantsTransacStatus.COMPLETE.name()));
    }
    ...
    public Flux<TransactionDto> findTransactionByProduct(String productId) {
        ...
        return transactionRepository.findByProductId(productId).map(AppUtils::entityToTransactionDto).filter(transactionDto -> transactionDto.getStatus().equalsIgnoreCase(ConstantsTransacStatus.COMPLETE.name()));
    }
    ...
    public Mono<TransactionDto> createTransactionDebit(Transaction transaction) {
        ...
        return filterProduct.flatMap(productDto -> productDto.getId() != null ?
                filterTransaction.filterTransactionCreate(transaction).flatMap(transactionRepository::insert)
